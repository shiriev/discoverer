...
namespace Discoverer.Logic.Tests.Generator
{
    public class HintGeneratorTests
    {
        ...
        {
            var gridMock = new Mock<IImmutableGrid<Region>>();
            gridMock.Setup(_ => _.Items)
                .Returns(
                    Enumerable.Range(0, itemCount)
                        .Select(_ => (new TestCoordinate { I = _} as ICoordinate, new Region(ETerrainType.Desert, EHabitatType.Bear, new Building(EColor.Black, EBuildingType.Monument))))
                        .ToArray());
            
            var functions = hintPositions.ToDictionary<KeyValuePair<EHint, int[]>, EHint, Func<IImmutableGrid<Region>, ICoordinate, bool>>(
                kv => kv.Key,
